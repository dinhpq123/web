...
const WF_BLOCK_DEFS = [
  // TRIGGERS — red
  { id:'trigger_iot',      cat:'trigger',     color:'var(--danger)', label:'IoT Cảnh báo',     desc:'Kích hoạt khi ngưỡng IoT vượt mức',     icon:_wfSvg('<path d="M5 12.55a11 11 0 0 1 14.08 0"/><path d="M1.42 9a16 16 0 0 1 21.16 0"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><circle cx="12" cy="20" r="1" fill="currentColor"/>') },
  { id:'trigger_schedule', cat:'trigger',     color:'var(--danger)', label:'Lịch hẹn',          desc:'Kích hoạt theo lịch cron/ngày giờ',      icon:_wfSvg('<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>') },
  { id:'trigger_report',   cat:'trigger',     color:'var(--danger)', label:'Báo cáo nộp',       desc:'Khi có báo cáo mới từ cộng đồng',        icon:_wfSvg('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/>') },
  { id:'trigger_webhook',  cat:'trigger',     color:'var(--danger)', label:'API Webhook',        desc:'HTTP POST từ hệ thống ngoài',             icon:_wfSvg('<path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/>') },
  { id:'trigger_manual',   cat:'trigger',     color:'var(--danger)', label:'Thủ công',           desc:'Kích hoạt bằng tay bởi người dùng',      icon:_wfSvg('<path d="M18 11V6a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v0"/><path d="M14 10V4a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v2"/><path d="M10 10.5V6a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v8"/><path d="M18 8a2 2 0 1 1 4 0v6a8 8 0 0 1-8 8h-2c-2.8 0-4.5-.86-5.99-2.34l-3.6-3.6a2 2 0 0 1 2.83-2.82L7 15"/>') },
  { id:'trigger_alert',    cat:'trigger',     color:'var(--danger)', label:'Cảnh báo hệ thống', desc:'Khi có alert mức BĐ2/BĐ3',               icon:_wfSvg('<path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>') },
  // CONDITIONS — amber
  { id:'cond_if',          cat:'condition',   color:'var(--warning-text)', label:'Điều kiện If/Else', desc:'Rẽ nhánh theo điều kiện',                icon:_wfSvg('<polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/>') },
  { id:'cond_threshold',   cat:'condition',   color:'var(--warning-text)', label:'So sánh ngưỡng',   desc:'So sánh giá trị với ngưỡng',             icon:_wfSvg('<line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/>') },
  { id:'cond_time',        cat:'condition',   color:'var(--warning-text)', label:'Thời gian',         desc:'Kiểm tra giờ/ngày trong tuần',           icon:_wfSvg('<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>') },
  { id:'cond_role',        cat:'condition',   color:'var(--warning-text)', label:'Kiểm tra Role',     desc:'Theo role/bộ phận',                      icon:_wfSvg('<path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>') },
  { id:'cond_and',         cat:'condition',   color:'var(--warning-text)', label:'AND / OR',          desc:'Kết hợp nhiều điều kiện',                icon:_wfSvg('<circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M6 21V9a9 9 0 0 0 9 9"/>') },
  // ACTIONS — green
  { id:'act_notify',       cat:'action',      color:'var(--primary)', label:'Gửi thông báo',    desc:'Push notification / in-app',             icon:_wfSvg('<path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/>') },
  { id:'act_email',        cat:'action',      color:'var(--primary)', label:'Gửi Email',         desc:'Email qua SMTP',                         icon:_wfSvg('<path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/>') },
  { id:'act_sms',          cat:'action',      color:'var(--primary)', label:'Gửi SMS',           desc:'SMS qua nhà mạng',                       icon:_wfSvg('<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/><line x1="9" y1="10" x2="9.01" y2="10"/><line x1="12" y1="10" x2="12.01" y2="10"/><line x1="15" y1="10" x2="15.01" y2="10"/>') },
  { id:'act_tts',          cat:'action',      color:'var(--primary)', label:'Phát loa TTS',      desc:'Đọc thông báo qua hệ thống loa',         icon:_wfSvg('<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/>') },
  { id:'act_report',       cat:'action',      color:'var(--primary)', label:'Tạo báo cáo',       desc:'Sinh file báo cáo tự động',              icon:_wfSvg('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>') },
  { id:'act_update_data',  cat:'action',      color:'var(--primary)', label:'Cập nhật dữ liệu', desc:'Ghi dữ liệu vào hệ thống',               icon:_wfSvg('<path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v14a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/>') },
  { id:'act_zalo',         cat:'action',      color:'var(--primary)', label:'Zalo ZNS',          desc:'Gửi tin Zalo tới nhóm/cá nhân',          icon:_wfSvg('<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/><line x1="8" y1="10" x2="16" y2="10"/><line x1="8" y1="14" x2="14" y2="14"/>') },
  { id:'act_gis_mark',     cat:'action',      color:'var(--primary)', label:'Đánh dấu GIS',     desc:'Tạo marker trên bản đồ GIS',             icon:_wfSvg('<path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>') },
  // AI BLOCKS — purple
  { id:'ai_analyze',       cat:'ai',          color:'var(--primary)', label:'Phân tích AI',     desc:'GPT phân tích dữ liệu đầu vào',          icon:_wfSvg('<path d="M12 2a10 10 0 1 0 10 10"/><path d="M12 12l4-4"/><circle cx="12" cy="12" r="2" fill="currentColor"/>') },
  { id:'ai_forecast',      cat:'ai',          color:'var(--primary)', label:'Dự báo xu hướng', desc:'Mô hình ML dự báo ngắn hạn',             icon:_wfSvg('<polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>') },
  { id:'ai_anomaly',       cat:'ai',          color:'var(--primary)', label:'Bất thường',       desc:'Phát hiện outlier trong chuỗi thời gian', icon:_wfSvg('<circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/>') },
  { id:'ai_classify',      cat:'ai',          color:'var(--primary)', label:'Phân loại AI',     desc:'Tự động gán mức độ/loại sự kiện',        icon:_wfSvg('<rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/>') },
  { id:'ai_generate_text', cat:'ai',          color:'var(--primary)', label:'Tạo văn bản AI',  desc:'Sinh nội dung báo cáo, thông báo',        icon:_wfSvg('<path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>') },
  { id:'ai_summarize',     cat:'ai',          color:'var(--primary)', label:'Tóm tắt AI',       desc:'Tóm tắt tài liệu/dữ liệu dài',           icon:_wfSvg('<line x1="21" y1="10" x2="7" y2="10"/><line x1="21" y1="6" x2="3" y2="6"/><line x1="21" y1="14" x2="3" y2="14"/><line x1="21" y1="18" x2="7" y2="18"/>') },
  // CONTROL — slate
  { id:'ctrl_delay',       cat:'control',     color:'var(--muted)', label:'Delay',             desc:'Chờ N giây/phút/giờ',                    icon:_wfSvg('<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>') },
  { id:'ctrl_loop',        cat:'control',     color:'var(--muted)', label:'Lặp lại',           desc:'Lặp N lần hoặc foreach',                 icon:_wfSvg('<polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/>') },
  { id:'ctrl_parallel',    cat:'control',     color:'var(--muted)', label:'Song song',          desc:'Chạy nhiều nhánh cùng lúc',              icon:_wfSvg('<line x1="12" y1="2" x2="12" y2="22"/><path d="M2 7h20"/><path d="M2 17h20"/>') },
  { id:'ctrl_end',         cat:'control',     color:'var(--muted)', label:'Kết thúc',           desc:'Hoàn thành workflow',                    icon:_wfSvg('<rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="9" x2="15" y2="15"/><line x1="15" y1="9" x2="9" y2="15"/>') },
  // INTEGRATION — cyan
  { id:'int_scada',        cat:'integration', color:'var(--primary)', label:'SCADA',             desc:'Gửi lệnh tới hệ thống SCADA',            icon:_wfSvg('<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/>') },
  { id:'int_api',          cat:'integration', color:'var(--primary)', label:'Gọi API',           desc:'HTTP GET/POST tới API ngoài',             icon:_wfSvg('<circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>') },
  { id:'int_db',           cat:'integration', color:'var(--primary)', label:'Truy vấn DB',       desc:'SQL query / NoSQL lookup',               icon:_wfSvg('<ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/>') },
];

const WF_CATS = [
  { id:'trigger',     label:'Trigger',    color:'var(--danger)' },
  { id:'condition',   label:'Điều kiện',  color:'var(--warning-text)' },
  { id:'action',      label:'Hành động',  color:'var(--primary)' },
  { id:'ai',          label:'AI Blocks',  color:'var(--primary)' },
  { id:'control',     label:'Điều phối',  color:'var(--muted)' },
  { id:'integration', label:'Tích hợp',   color:'var(--primary)' },
];
